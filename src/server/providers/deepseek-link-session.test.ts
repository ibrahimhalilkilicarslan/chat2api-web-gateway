...
describe('DeepSeekLinkSessionRegistry', () => {
  it('binds admin reads to the creating session and never exposes the secret', () => {
    ...
    expect(JSON.stringify(created.session)).not.toContain(created.secret)
  })
  ...
  it('accepts a capability once, serializes validation and supports idempotent completion', () => {
    ...
    expect(registry.claim(created.session.id, 'wrong-secret', 2_001)).toBeUndefined()
    expect(registry.claim(created.session.id, created.secret, 2_001)).toMatchObject({
      kind: 'ready',
      name: 'Dedicated account',
      email: 'account@example.com',
      dailyLimit: 500,
    })
    expect(registry.claim(created.session.id, created.secret, 2_002)).toMatchObject({
      kind: 'busy',
    })

    registry.complete(created.session.id, 'account-id')
    expect(registry.claim(created.session.id, created.secret, 2_003)).toMatchObject({
      kind: 'complete',
  ...
  it('returns failed validation to waiting state and expires short-lived sessions', () => {
    ...
    registry.claim(created.session.id, created.secret, 3_001)
    ...
    expect(registry.claim(created.session.id, created.secret, created.session.expiresAt)).toBeUndefined()
  })
})
